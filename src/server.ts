...
import https, { Agent, AgentOptions, RequestOptions } from "https";
...
// TLS/HTTP client tuning
const agentOpts: AgentOptions = { keepAlive: true, family: 4 };
if (process.env.TLS12_ONLY === "1") {
  (agentOpts as any).secureProtocol = "TLSv1_2_method";
}
const agent: Agent = new https.Agent(agentOpts);

// Choose Node or Fetch HTTP client for Stripe (set STRIPE_HTTP_CLIENT=fetch to try fetch)
const httpClient =
...
const ZOHO_FIELD_AMOUNT = process.env.ZOHO_FIELD_AMOUNT;
const ZOHO_FIELD_PAYMENT_METHOD = process.env.ZOHO_FIELD_PAYMENT_METHOD;
const HAS_ZOHO = !!(ZOHO_CLIENT_ID && ZOHO_CLIENT_SECRET && ZOHO_REFRESH_TOKEN);
...
// 1) Basic Stripe account reachability (auth via Stripe SDK)
...
// 2) DNS + unauthenticated HTTPS probes
...
// 3) Direct HTTPS to Stripe with Authorization header (bypasses Stripe SDK)
app.get("/api/diag/stripe-direct", async (_req: Request, res: Response) => {
  const result = await rawHttpsGetAuth("https://api.stripe.com/v1/accounts", agent, STRIPE_SECRET_KEY!);
  res.status(result.status ?? (result.ok ? 200 : 500)).json(result);
});

function rawHttpsGet(urlStr: string, ag: Agent): Promise<{ ok: boolean; status?: number; error?: string }> {
...
function rawHttpsGetAuth(
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(opts, (resp) => {
      ...
      resp.on("end", () => resolve({ ok: true, status: resp.statusCode ?? 0, body: data.slice(0, 300) }));
    });
    req.on("error", (err) => resolve({ ok: false, error: err.message }));
    req.on("timeout", () => req.destroy(new Error("timeout")));
    req.end();
  ...
  try {
    ...
    if (HAS_ZOHO) {
      try {
        ...
        console.warn("Zoho upsert skipped:", e instanceof Error ? e.message : String(e));
      }
    }
    ...
    const pi = await stripe.paymentIntents.create({
      ...
      metadata: { zoho_lead_id: zohoLeadId, product_name: product, source: "vetletters_site" },
...
async function onPaymentSucceeded(pi: Stripe.PaymentIntent): Promise<void> {
  try {
    ...
    const access = await getZohoAccessToken();
    await updateZohoLeadPaid(access, leadId, { product_name: product, amount, method, receipt });
  } catch (e: unknown) {
...
async function getZohoAccessToken(): Promise<string> {
  ...
  const params = new URLSearchParams({
    refresh_token: String(ZOHO_REFRESH_TOKEN),
    client_id: String(ZOHO_CLIENT_ID),
    client_secret: String(ZOHO_CLIENT_SECRET),
...
async function zohoReq<T = unknown>(
  ...
  if (!r.ok) {
    const msg = (data as any)?.message || (data as any)?.data?.[0]?.message || `Zoho ${method} ${path} failed (${r.status})`;
...
async function updateZohoLead(accessToken: string, id: string, fields: Record<string, unknown>): Promise<void> {
...
async function createZohoLead(accessToken: string, leadFields: Record<string, unknown>): Promise<string> {
  const resp = await zohoReq<any>(accessToken, "POST", "/crm/v3/Leads", { data: [leadFields] });
  const id: string | undefined = resp?.data?.[0]?.details?.id;
  if (!id) throw new Error("Zoho create lead failed");
  return id;
}

async function addZohoNote(accessToken: string, leadId: string, title: string, content: string): Promise<void> {
  await zohoReq(accessToken, "POST", "/crm/v3/Notes", {
    data: [{ Note_Title: title, Note_Content: content, Parent_Id: leadId, se_module: "Leads" }],
  });
}

async function upsertZohoLead(accessToken: string, lead: Lead, product: string): Promise<string> {
...
async function updateZohoLeadPaid(
  ...
  const today = new Date().toISOString().slice(0, 10);
  const updates: Record<string, unknown> = { Order_Status: "Paid", Order_Date: today, Product_Name: info.product_name };
