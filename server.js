...
/* =============================== Handlers ================================== */
// Use latest_charge instead of pi.charges (fixes TS error).
async function getLatestCharge(pi: Stripe.PaymentIntent): Promise<any | null> {
  try {
    if (!pi.latest_charge) return null;
    if (typeof pi.latest_charge === "string") {
      // Retrieve the Charge to get payment_method_details & receipt_url
      const ch = await stripe.charges.retrieve(pi.latest_charge);
      return ch as any; // cast to simplify TS (Stripe.Response<Charge> compatible)
    }
    return pi.latest_charge as any; // already expanded
  } catch {
    // Fallback: try re-fetching PI with expand
    try {
      ...
      if (typeof full.latest_charge === "string") {
        const ch = await stripe.charges.retrieve(full.latest_charge);
        return ch as any;
      }
      return full.latest_charge as any;
    } catch {
      return null;
    }
  }
...
async function getZohoAccessToken(): Promise<string> {
  if (!hasZoho) throw new Error("Zoho credentials missing");
  const url =
